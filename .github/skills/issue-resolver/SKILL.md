---
name: issue-resolver
description: Specialized skill for investigating and resolving community-reported .NET MAUI issues through hands-on testing and implementation. Use this skill when the user asks to "fix issue #XXXXX", "resolve bug #XXXXX", or "implement fix for #XXXXX".
license: MIT
metadata:
  version: "1.0"
...
- ❌ "Write UI tests" without fixing a bug → Use `uitest-coding`
- ❌ Just discussing issue without implementing → Analyze directly, no skill needed

**Note**: This skill does full issue resolution lifecycle: reproduce → investigate → fix → test → PR.

---

## ⚠️ MANDATORY WORKFLOW ENFORCEMENT

This skill has TWO HARD STOPS that MUST be followed:
...
**PR Reviewer Validation**: The `pr-reviewer` skill validates that PRs follow these standards before approval.
